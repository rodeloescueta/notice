<template>
  <v-app id="keep">
    <v-app-bar
        ...
        >
        <span class="title font-weight-bold" style="color: #008B8B;">HRD Notice @admin</span>
        <v-spacer />
        <v-btn dark color="#008B8B" @click="noticeDialog = !noticeDialog">
            <v-icon>mdi-tab-plus</v-icon> notice
        </v-btn>
        <v-btn icon fab @click="logout()"><v-icon>mdi-logout-variant</v-icon></v-btn>
    </v-app-bar>
    ...
    >
        <v-card class="pa-4">
            ...
            <v-textarea v-if="userInfo.user === '0320'"
                name="input-7-1"
...
import { VueEditor } from "vue2-editor";
  export default {
    ...
    data: () => ({
        ...
        smsUrl: 'https://igu4xu2bea.execute-api.us-east-2.amazonaws.com/prod/sendSMS/',
        content: "",
        noticeTitle: "",
        noticeDialog: false,
        noticeData: [],
    ...
    methods: {
        sendSms(){
            ...
            let arrayLinks = this.numArray.map(n => `${this.smsUrl}?message=New Information @ website url here  regarding ${this.noticeTitle}&number=${n}&subject=test`)
            // console.log(arrayLinks)
            axios.all(arrayLinks.map(l => axios.get(l)))
        ...
        onDelete(val){
            let r = confirm(`Are you sure you want to delete notice "${val.noticeTitle}"`)
            const index = this.noticeData.indexOf(val)
            if(r==true){
                let url = `${this.api}api/record`
                axios.delete(url,{data:{noticeTitle: val.noticeTitle, dateReleased: val.dateReleased}}).then(() => {
