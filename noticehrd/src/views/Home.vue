<template>
  <v-app id="inspire">
    ...
    <v-app-bar app clipped-left dense flat color="light-green lighten-5">
      ...
      >
      <v-spacer />
      <v-btn dark color="#008B8B" v-if="!showAll" @click="onShowAll()">
        ...
        <v-row justify="center">
          <template v-if="showAll">
            <template v-for="(notice, i) in noticeData">
              <v-container :key="i + 'a'">
                <v-card class="pa-2" scrollable @click="view(notice)">
                  <v-row>
                    <v-col class="pt-0" cols="12">
                      <v-card-text>
                        ...
                        <p style="color: #757575;">
                          {{
                            [viewNotice.dateReleased, "MM-DD-YYYY HH:mm:ss"]
...
export default {
  ...
  methods: {
    ...
    view(val) {
      this.viewNotice = val;
      this.$router.push(`/${val.dateReleased}`);
      this.showAll = false;
    },
    onView(val) {
      this.viewNotice = val;
